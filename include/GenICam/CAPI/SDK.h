...
typedef struct GENICAM_Camera
{
    ...
    /// \~chinese
    /// \brief 保存相机的配置到指定的位置。同名文件已存在时，覆盖。
    /// \param [in] thiz 本接口指针
    /// \param [in] pXmlFile 文件名，如：D:\\config.xml
    /// \retval 0        成功
    /// \retval 非零     失败
    /// \~english save the configuration of the camera. Overwrite the file if exists.
    /// \param [in] thiz this pointer
    /// \param [in] pXmlFile The full path name of the property file(xml).  e.g. D:\\config.xml
    /// \retval 0        success
    /// \retval non-zero fail
    int32_t (*saveDeviceCfg)(struct GENICAM_Camera *thiz, const char* pXmlFile);

    /// \~chinese
    /// \brief 从文件加载相机xml配置
    /// \param [in] thiz 本接口指针
    /// \param [in] pXmlFileName xml文件路径,如：D:\\config.xml 或 D:\\config.mvcfg
    /// \param [out] errorList  加载失败的属性名列表。一维表示属性个数，最大值为MAX_PARAM_CNT。二维是属性名，字符长度最长为MAX_STRING_LENTH。如：char errorList[MAX_PARAM_CNT][MAX_STRING_LENTH]
    /// \param [in/out] pErrCnt in:申请的errorList的一维个数，最大为 MAX_PARAM_CNT; out:实际加载失败的属性个数.
    /// \retval 0       成功
    /// \retval 非零    失败
    /// \~english load the configuration of the camera
    /// \param [in] thiz this pointer
    /// \param [in] pXmlFileName The full path name of the property file(xml). e.g. D:\\config.xml or D:\\config.mvcfg
    /// \param [out] errorList the list of load failed properties. maximum length of chars is MAX_STRING_LENTH. e.g. char errorList[MAX_PARAM_CNT][MAX_STRING_LENTH]
    /// \param [in/out] pErrCnt in:the size of errorList's one dimension, maximum number is MAX_PARAM_CNT; out: the count of load failed properties.
    /// \retval 0        success
    /// \retval non-zero fail
    int32_t (*loadDeviceCfg)(struct GENICAM_Camera *thiz, const char* pXmlFileName, char errorList[][MAX_STRING_LENTH], uint32_t *pErrCnt);
	...
	uint32_t (*writeUARTData)(struct GENICAM_Camera *thiz, const void* pBuffer, uint32_t dwCount);
	...
	uint32_t (*readUARTData)(struct GENICAM_Camera *thiz, void* pBuffer, uint32_t dwCount);

	uint32_t		reserved[11];	///< \~chinese 保留  	///< \~english reserved field
}GENICAM_Camera;
/// @}
...
typedef struct GENICAM_System
{
	...
	/// \~chinese
	/// \brief 根据相机的DeviceUserID获取 Camera 对象指针
	/// \param [in] thiz 本接口指针
	/// \param [in] pDeviceUserID 相机的DeviceUserID
	/// \return 返回Camera指针
	/// 若传入的pDeviceUserID不存在或者非法，则返回空指针.
	/// 若设备列表中存在重复DeviceUserID的相机，则返回列表中第一个对应的DeviceUserID的相机指针.
	/// \~english
	/// \brief get the pointer of camera object according to camera DeviceUserID
	/// \param [in] thiz this pointer
	/// \param [in] pDeviceUserID camera DeviceUserID
	/// \return Success:camera pointer, Failure:NULL
	/// if the parameter pDeviceUserID does not exist or is invalid, return NULL
	/// if the DeviceUserID in the device list is duplicated, return the first camera pointer that matches the DeviceUserID.
	GENICAM_Camera* (*getCameraByDeviceUserID)(struct GENICAM_System *thiz, const char * pDeviceUserID);

	/// \~chinese
	/// \brief 根据相机的IP地址获取 Camera 对象指针，仅限Gige设备使用.
	/// \param [in] thiz 本接口指针
	/// \param [in] pIPAddress 相机的IP地址
	/// \return 返回Camera指针,若传入的pIPAddress不存在或者非法，则返回空指针
	/// \~english
	/// \brief get the pointer of camera object according to camera IP address. Only for Gige device.
	/// \param [in] thiz this pointer
	/// \param [in] pIPAddress camera IP address
	/// \return Success:camera pointer, Failure:NULL
	GENICAM_Camera* (*getCameraByIP)(struct GENICAM_System *thiz, const char * pIPAddress);
	...
	uint32_t		reserved[22];	///< 保留	///< \~english reserved field
}GENICAM_System;
...
typedef struct GENICAM_PCIEStreamStatsInfo
{
	//帧相关统计
	uint32_t    imageError;				///< 图像错误的帧数
	uint32_t    lostPacketBlock;		///< 丢包的帧数
	uint32_t    overrideBlock;			///< 覆盖的帧数
	uint32_t	reserved0[9];			///< 预留

	//正常帧统计
	uint32_t    imageReceived;			///< 正常获取的帧数
	double      fps;                    ///< 帧率
	double      bandwidth;              ///< 带宽(Mbps)
	uint32_t    reserved[8];            ///< 预留

}GENICAM_PCIEStreamStatsInfo;

typedef struct GENICAM_U3VStreamStatsInfo
{
	//帧相关统计
	uint32_t    imageError;				///< 图像错误的帧数
	uint32_t    lostPacketBlock;		///< 丢包的帧数
...
}GENICAM_U3VStreamStatsInfo;

typedef struct GENICAM_GigEStreamStatsInfo
{
	//包相关统计
	uint32_t	reserved0[10];			///< 预留

	//帧相关统计
	uint32_t    imageError;				///< 图像错误的帧数
	uint32_t    lostPacketBlock;		///< 丢包的帧数
	uint32_t    overrideBlock;			///< 覆盖的帧数
	uint32_t    reserved1[3];			///< 预留
	uint32_t	reserved2[5];			///< 预留
...
}GENICAM_GigEStreamStatsInfo;

typedef struct GENICAM_StreamStatisticsInfo
{
	uint32_t nCameraType;		/// 相机类型

	union
	{
		GENICAM_PCIEStreamStatsInfo pcieStatisticsInfo;
		GENICAM_U3VStreamStatsInfo u3vStatisticsInfo;
		GENICAM_GigEStreamStatsInfo gigeStatisticsInfo;
	};
}GENICAM_StreamStatisticsInfo;
...
typedef struct GENICAM_StreamSource
{
	...
	/// \param [in] thiz 本接口指针
	/// \param [in] packetNum 单次重传最大包个数
	/// \return 设置是否成功,0表示成功，-1表示失败
	/// \~english
	/// \brief set the single resend maximum packet number, just for GigE devices
	/// \param [in] thiz this pointer
	/// \param [in] the value of single resend maximum packet number
	/// \return success:0, fail:-1
	int32_t(*setSingleResendMaxPacketNum)(struct GENICAM_StreamSource *thiz, uint32_t maxPacketNum);

	/// \~chinese
	/// \brief 设置同一帧最大丢包的数量,仅对Gige设备有效
	/// \param [in] thiz 本接口指针
	/// \param [in] lostPacketNum 最大丢包的数量
	/// \return 设置是否成功,0表示成功，-1表示失败
	/// \~english
	/// \brief set the maximum lost packet number, just for GigE devices
	/// \param [in] thiz this pointer
	/// \param [in] the value of maximum lost packet number
	/// \return success:0, fail:-1
	int32_t(*setMaxLostPacketNum)(struct GENICAM_StreamSource *thiz, uint32_t maxLostPacketNum);

	/// \~chinese
	/// \brief 获取流统计信息
	/// \param [in] ppStreamStatsInfo 流统计信息数据
	/// \return 设置是否成功,0表示成功，-1表示失败
	/// \~english
	/// \brief get stream statistics infomation
	/// \param [in] the value of stream statistics infomation,
	/// \return success:0, fail:-1
	int32_t(*getStatisticsInfo)(struct GENICAM_StreamSource *thiz, GENICAM_StreamStatisticsInfo** ppStreamStatsInfo);
	...
	/// \return success:0, fail:-1
	int32_t(*resetStatisticsInfo)(struct GENICAM_StreamSource *thiz);
