...
public:
    ...
    /// \param [in] pXmlFile 导出的相机配置文件全名（含路径），如：D:\\config.xml
    /// \retval 0        成功
    /// \retval 非零     失败
    ...
    virtual int32_t saveDeviceCfg(const char* pXmlFile) = 0;

    /// \~chinese
    /// \brief 从文件加载相机xml配置
    /// \param [in] pXmlFileName 相机配置（xml）文件全名（含路径），如：D:\\config.xml 或 D:\\config.mvcfg
    /// \param [out] 加载失败的属性名列表
    /// \retval 0       成功
    /// \retval 非零    失败
    ...
    /// \param [out] the list of load failed properties name.
    /// \retval 0        success
    /// \retval non-zero fail
    virtual int32_t loadDeviceCfg(const char* pXmlFileName, Infra::TVector<Infra::CString>& vErrorList) = 0;
};
