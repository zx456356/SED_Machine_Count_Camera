...
VisionArcItem::VisionArcItem(bool bEdit, bool color_enable, QColor borderColor, QColor selectedColor, QColor brushColor, VisionItem *parent) : VisionItem(parent)
{
    if(color_enable){
...
void VisionArcItem::setCoordinate_rela(Coordinate_Struct stru)
{
...
bool VisionArcItem::detailData(QPointF sP, QPointF mP, QPointF fP, QPointF center)
{
    //三点确定一个圆,硬解，可得到表达式（三点一线的时候，则无法形成圆）
    double x1 = sP.x(), x2 = mP.x(), x3 = fP.x();
    double y1 = sP.y(), y2 = mP.y(), y3 = fP.y();
