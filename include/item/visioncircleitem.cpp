...
void VisionCircleItem::setCircle(qreal x, qreal y, qreal width, qreal height)
{
    m_width = width;
    m_height = height;
    m_x = x;
...
//    QPointF pointF = QPointF(m_x,m_y);
//    pointF = convert_coordinate(stru,m_itemCoordinate,pointF);
...
void VisionCircleItem::setCircle(QPointF center, qreal r)
{
    m_width = 2*r;
    m_height = 2*r;
    m_x = center.x() - r;
...
void VisionCircleItem::setCircle(QRectF rf)
{
    m_width = rf.width();
    m_height = rf.height();
    m_x = rf.x();
...
bool VisionCircleItem::getPosInArea(qreal x, qreal y)
{
...
QVector<QPointF> VisionCircleItem::getPoints()
{
...
VGCircle2D VisionCircleItem::getData(){
    //转换数据
    Coordinate_Struct stru;
    stru.angle = 0;
    stru.dx = 0;
    ...
    QPointF pointF = QPointF(m_x+m_width/2,m_y+m_height/2);
    pointF = convert_coordinate(m_itemCoordinate,stru,pointF);

    qreal x = pointF.x()-m_width/2;
    qreal y = pointF.y()-m_height/2;
    ...
    vgPoint.x = x+m_width/2;
    vgPoint.y = y+m_height/2;
...
void VisionCircleItem::setCoordinate_rela(Coordinate_Struct stru)
{
    QPointF pointF = QPointF(m_x+m_width/2,m_y+m_height/2);
    pointF = convert_coordinate(m_itemCoordinate,stru,pointF);
    ...
    m_x = pointF.x()-m_width/2;
    m_y = pointF.y()-m_height/2;
...
void VisionCircleItem::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
    Q_UNUSED(widget)
    Q_UNUSED(option)
...
QRectF VisionCircleItem::boundingRect() const
{
    QRectF rf = QRectF(-5,-5,m_width+10,m_width+10);
    return rf;
}

void VisionCircleItem::mousePressEvent(QGraphicsSceneMouseEvent *event)
{
...
void VisionCircleItem::mouseMoveEvent(QGraphicsSceneMouseEvent *event)
{
    if(!m_bEdit)
        return;
...
void VisionCircleItem::initItem()
{
    m_lstRect.clear();
...
void VisionCircleItem::updateItem()
{
    qreal x;qreal y;
    for(int i=0;i<m_lstRect.count();i++){
        if(i == 0){
...
QList<QPointF> VisionCircleItem::rfToPoint(QRectF rf)
{
...
QRectF VisionCircleItem::pointsToRf(QList<QPointF> lstPointF)
{
...
QRectF VisionCircleItem::detailMove(int index, QPointF p)
{
    qreal w;
...
void VisionCircleItem::slotIndex(int index)
{
