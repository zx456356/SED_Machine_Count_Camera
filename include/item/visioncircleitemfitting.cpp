...
VisionCircleItemFitting::VisionCircleItemFitting(bool edit, qreal length, bool color_enable, QColor borderColor, QColor selectedColor, QColor brushColor, VisionItem *parent) : VisionItem(parent)
{
    if(color_enable){
...
VGCircleFittingField VisionCircleItemFitting::getData(){
    //转换数据
    Coordinate_Struct stru;
    stru.angle = 0;
    stru.dx = 0;
    ...
    vgCircleFitting.optional = VG_ENABLE;
...
void VisionCircleItemFitting::setCoordinate_rela(Coordinate_Struct stru)
{
    QPointF pointF = QPointF(m_x+m_width/2,m_y+m_height/2);
    pointF = convert_coordinate(m_itemCoordinate,stru,pointF);
...
void VisionCircleItemFitting::updateLength(qreal length)
{
    m_length = length;
