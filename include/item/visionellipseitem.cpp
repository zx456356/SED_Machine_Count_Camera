...
VisionEllipseItem::VisionEllipseItem(bool edit, bool rotation, bool color_enable, QColor borderColor, QColor selectedColor, QColor brushColor, VisionItem *parent):VisionItem(parent)
{
    if(color_enable){
...
void VisionEllipseItem::setRect(qreal x, qreal y, qreal width, qreal height, qreal angle)
{
    m_width = width;
    m_height = height;
    m_x = x;
...
void VisionEllipseItem::setRect(QRectF rf, qreal angle)
{
    m_width = rf.width();
    m_height = rf.height();
    m_x = rf.x();
...
VGRectangle2D VisionEllipseItem::getData(){
    ...
    qreal x = centerPointF.x()-w/2;
    qreal y = centerPointF.y()-h/2;
    Q_UNUSED(x)
    Q_UNUSED(y)
...
void VisionEllipseItem::setCoordinate_rela(Coordinate_Struct stru)
{
