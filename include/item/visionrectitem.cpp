...
//    m_pointF1 = convert_coordinate(stru,m_itemCoordinate,m_pointF1);
//    m_pointF2 = convert_coordinate(stru,m_itemCoordinate,m_pointF2);
//    m_pointF3 = convert_coordinate(stru,m_itemCoordinate,m_pointF3);
//    m_pointF4 = convert_coordinate(stru,m_itemCoordinate,m_pointF4);
//    m_angle = m_angle + (m_itemCoordinate.angle - stru.angle);
//    updateBoundingRect();

    this->setPos(m_x,m_y);

    if(m_lstRect.count() <= 0)
        initItem();
    ...
    if(m_bRotation){
        ...
        arrowsItem = new VisionArrows_Rotate(m_width/2,m_height/2,w,h,QColor(255,0,0),this);
...
VGRectangle2D VisionRectItem::getData(){
    //转换数据
    Coordinate_Struct stru;
    stru.angle = 0;
    stru.dx = 0;
    ...
    QPointF p1 = m_pointF1;
    QPointF p2 = m_pointF2;
    QPointF p3 = m_pointF3;
    QPointF p4 = m_pointF4;
    p1 = convert_coordinate(m_itemCoordinate,stru,p1);
    p2 = convert_coordinate(m_itemCoordinate,stru,p2);
    p3 = convert_coordinate(m_itemCoordinate,stru,p3);
    p4 = convert_coordinate(m_itemCoordinate,stru,p4);

//    qDebug()<<m_pointF1<<m_pointF2<<m_pointF3<<m_pointF4<<p1<<p2<<p3<<p4;
//    qDebug()<<m_x<<m_y;
    QPointF centerPointF = QPointF((p1.x()+p3.x())/2,(p1.y()+p3.y())/2);
    qreal w = sqrt((p1.x()-p2.x())*(p1.x()-p2.x())+(p1.y()-p2.y())*(p1.y()-p2.y()));
    qreal h = sqrt((p1.x()-p4.x())*(p1.x()-p4.x())+(p1.y()-p4.y())*(p1.y()-p4.y()));
    Q_UNUSED(w)
    Q_UNUSED(h)
    qreal x = centerPointF.x()-m_width/2;
    qreal y = centerPointF.y()-m_height/2;

    qreal angle = m_angle + (stru.angle - m_itemCoordinate.angle);
    //已经在旋转后进行了换算
    //获取中心点，两条对角线的交点，
    qDebug()<<x<<y<<p1.x()<<p1.y();
    ...
    vgRect2D.angle = angle;

    return vgRect2D;
}

void VisionRectItem::setCoordinate_rela(Coordinate_Struct stru)
{
    m_pointF1 = convert_coordinate(m_itemCoordinate,stru,m_pointF1);
    m_pointF2 = convert_coordinate(m_itemCoordinate,stru,m_pointF2);
    m_pointF3 = convert_coordinate(m_itemCoordinate,stru,m_pointF3);
    m_pointF4 = convert_coordinate(m_itemCoordinate,stru,m_pointF4);

    m_angle = m_angle + (stru.angle - m_itemCoordinate.angle);
...
void VisionRectItem::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
    ...
    //在非编辑模式下,不存在旋转标志的时候，绘制正方向的标志
    if(!m_bRotation && !m_bEdit){
        //首先计算出不旋转的正方向的标志
        QPointF p1 = m_pointF1 - originPointF; //对应左上顶点
        QPointF p1_0 = p1;
        QPointF p2 = m_pointF2 - originPointF;
        QPointF p4 = m_pointF4 - originPointF;
        qreal leng12 = sqrt((p1.x()-p2.x())*(p1.x()-p2.x()) + (p1.y()-p2.y())*(p1.y()-p2.y()));//点1和点2的长度
        qreal leng14 = sqrt((p1.x()-p4.x())*(p1.x()-p4.x()) + (p1.y()-p4.y())*(p1.y()-p4.y()));//点1和点4的长度
        ...
        QPointF p2_0 = p1+QPointF(leng*5/100,0); //和边对应的点，左上顶点的右部点

        QPointF p3_0 = p1+QPointF(leng*5/200,-leng*5/200);  //左上顶点对应的对角点

        QPointF p4_0 = p1-QPointF(0,leng*5/200);   //左上顶点的上部点

        //筛选数据，处理畸形，以水平的线为参考标准，竖直的不参考，比例为1：2的直角梯形
        QVector<QPointF> points_0;
        points_0.append(p1_0-p1_0);
        points_0.append(p2_0-p1_0);
        points_0.append(p3_0-p1_0);
        points_0.append(p4_0-p1_0);
        painter->setBrush(QBrush(m_borderColor));
        painter->translate(p1_0);
        painter->rotate(m_angle);
        painter->drawPolygon(points_0);
    }
}
...
QRectF VisionRectItem::boundingRect() const
{
    ...
    //    QRectF rf = QRectF(-5-10,-5-10,r+20,r+20);
        ...
        //非编辑特殊处理
        qreal minX = m_pointF1.x();
        qreal minY = m_pointF1.y();
        ...
        vec_point.push_back(m_pointF4);
        QPolygonF polygonF = QPolygonF(vec_point);
        QRectF rf(minX-m_x,minY-m_y,polygonF.boundingRect().width(),polygonF.boundingRect().height());
...
//        qDebug()<<"mousePressEvent out out out"<<this->boundingRect();
        setSelectedStatus(false);
//        emit signal_clicked(this,false,false,event->scenePos().x(),event->scenePos().y());
//        event->ignore();
    }else{
//        qDebug()<<"mousePressEvent in in in"<<this->boundingRect();
        setSelectedStatus(true);
        lastPoint = event->scenePos();
...
//    QGraphicsItem::mouseReleaseEvent(event);
    ...
    if(!getPosInArea(event->scenePos().x(),event->scenePos().y())){
//        qDebug()<<"mouseReleaseEvent out out out"<<event->scenePos();
        emit signal_clicked(this,true,false,event->scenePos().x(),event->scenePos().y());
//        event->ignore();
    }else{
//        qDebug()<<"mouseReleaseEvent in in in"<<event->scenePos();
        emit signal_clicked(this,true,true,event->scenePos().x(),event->scenePos().y());
        QGraphicsItem::mouseReleaseEvent(event);
    }
    this->scene()->update();
...
bool bHoverLeaveStatus = false;
bool bHoverEnterStatus = false;
void VisionRectItem::hoverMoveEvent(QGraphicsSceneHoverEvent *event)
{
//    if(getPosInArea(event->lastScenePos().x(),event->lastScenePos().y())){
////        VisionItem::hoverLeaveEvent(event);
//        if(bHoverEnterStatus == false){
////            this->setZValue(5);
//            emit signal_hoverEnterEvent(this);
//        }
//        bHoverLeaveStatus = false;
//        bHoverEnterStatus = true;
//    }else{
//        if(bHoverLeaveStatus == false){
////            this->setZValue(0);
//            emit signal_hoverLeaveEvent(this);
//        }
//        bHoverLeaveStatus = true;
//        bHoverEnterStatus = false;
//    }
    QGraphicsItem::hoverMoveEvent(event);
    event->ignore();  //sceneEvent是拦截事件，调用ignore和accept都是无效的
}

void VisionRectItem::hoverEnterEvent(QGraphicsSceneHoverEvent *event)
{
    if(!m_bEdit)
        emit signal_hoverEnterEvent(this);
//    if(getPosInArea(event->lastScenePos().x(),event->lastScenePos().y())){
//        qDebug()<<"Test hoverEnterEvent";
//        emit signal_hoverEnterEvent(this);
//    }else{
//        qDebug()<<"Test hoverEnterEvent111111";
//        emit signal_hoverEnterEvent(this);
//    }
//    emit signal_hoverEnterEvent(this);
...
void VisionRectItem::hoverLeaveEvent(QGraphicsSceneHoverEvent *event)
{
    if(!m_bEdit)
        emit signal_hoverLeaveEvent(this);
...
{
    for(int i=0;i<m_lstRect.count();i++){
        //可能导致内存泄漏，但是在逻辑上就去避免了m_lstRect存在item，还需要再次调用initItem
        m_lstRect.at(i)->hide();
    }
    m_lstRect.clear();
